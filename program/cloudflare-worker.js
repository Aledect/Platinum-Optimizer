/**
 * Cloudflare Worker for Platinum+ Optimizer
 * Connects to GitHub Raw to serve website, wiki, and WPF GUI program installer.
 */

const GITHUB_RAW_BASE = 'https://raw.githubusercontent.com/Aledect/Platinum-Optimizer/main';
...
export default {
  async fetch(request, env, ctx) {
    ...
    // 1. Route /program or /program.ps1 -> Serve PowerShell Launcher Script
    if (pathname === '/program' || pathname === '/program.ps1' || pathname === '/program/') {
      const psScript = getPowerShellLauncherScript();
    ...
    // 2. Try serving from Cloudflare Static Assets first (if deployed)
    if (env.ASSETS) {
      let targetPath = pathname;
      if (pathname === '/' || pathname === '') targetPath = '/website/index.html';
      else if (pathname === '/wiki' || pathname === '/wiki/') targetPath = '/wiki/index.html';
      else if (pathname === '/website' || pathname === '/website/') targetPath = '/website/index.html';

      const assetRes = await env.ASSETS.fetch(new Request(new URL(targetPath, request.url), request));
      if (assetRes.status !== 404) {
        return assetRes;
      }
    }

    // 3. Fallback: Worker connects directly to GitHub Raw to fetch files
    let githubPath = pathname;
    if (pathname === '/' || pathname === '') githubPath = '/website/index.html';
    else if (pathname === '/wiki' || pathname === '/wiki/') githubPath = '/wiki/index.html';
    else if (pathname === '/website' || pathname === '/website/') githubPath = '/website/index.html';
    else if (!pathname.startsWith('/website/') && !pathname.startsWith('/wiki/') && !pathname.startsWith('/program/') && !pathname.startsWith('/update/')) {
      githubPath = `/website${pathname}`;
    }

    const githubUrl = `${GITHUB_RAW_BASE}${githubPath}`;
...
function getPowerShellLauncherScript() {
  return `# Platinum+ Optimizer Installer Bootstrapper
