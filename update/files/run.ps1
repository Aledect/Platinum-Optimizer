# Platinum+ Optimizer - Auto-Elevate Administrator Check
$currentPrincipal = New-Object Security.Principal.WindowsPrincipal([Security.Principal.WindowsIdentity]::GetCurrent())
if (-not $currentPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    $scriptFile = if ($PSCommandPath) { $PSCommandPath } elseif ($MyInvocation.MyCommand.Path) { $MyInvocation.MyCommand.Path } else { "$PSScriptRoot\run.ps1" }
...
# Also hide the console window if running in an interactive session
try {
...
try { $ui.FindName("IMG_DEBLOAT_TAB_MSAPPS").Source = $global:DebloatIco } catch {}
...
$ui.Add_Loaded({
    ...
    $transitionTimer.Add_Tick({
        ...
        Initialize-PlatinumShell -LogoPath "$ModuleRoot\ico\logo.png"
